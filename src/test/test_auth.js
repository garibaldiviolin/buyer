...
describe('JWT Authentication Tests', function() {
  ...
  describe('## Authenticate using token without expiration ', function() {
    it('should NOT accept token', function(done) {
      const token_without_exp =
        'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6IjVj' +
        'ZWIyNzJjMjg1OWEyM2U5MGM5MWY4OCJ9.zBuVQalol6nVSYCa' +
        'CMN_IGIcAwymPGDBRGA8dhDVYWk';
      request(app)
        .get('/products/list')
        .set('Authorization', 'Bearer ' + token_without_exp)
        .send()
        .end(function(err, res) {
          expect(res.statusCode).to.equal(401);
          expect(res.body).to.deep.equal({});
  ...
  describe('## Authenticate using token with invalid id ', function() {
    it('should NOT accept token', function(done) {
      ...
      request(app)
        .get('/products/list')
        .set('Authorization', 'Bearer ' + invalid_id_token)
        .send()
        .end(function(err, res) {
          expect(res.statusCode).to.equal(401);
          expect(res.body).to.deep.equal({});
