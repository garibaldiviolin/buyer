    ...
    it('should create a product', function(done) {
      request(app)
        ...
        .end(function(err, res) {
          ...
          const product_query = Product.findById(res.body._id, function(
            err,
            object
          ) {
            if (err) done(err);
  ...
  describe('## Update product ', function() {
    it('should update a product', function(done) {
      ...
      product
        ...
        .then(doc => {
          request(app)
            .put('/products/' + doc._id.toString() + '/update')
            .set('Authorization', 'Bearer ' + token)
            .send(new_values)
            .end(function(err, res) {
              var productsList = {
              ...
              const product_query = Product.findById(doc._id, function(
                err,
                object
              ) {
                if (err) done(err);
              }).then(updated_product => {
                expect(updated_product._id.toString()).to.equal(
    ...
    it('should NOT find or update a product', function(done) {
      ...
      request(app)
        .put('/products/INVALID_ID/update')
        .set('Authorization', 'Bearer ' + token)
        .send(new_values)
        .end(function(err, res) {
          expect(res.statusCode).to.equal(404);
  ...
  describe('## Delete product ', function() {
    it('should delete a product', function(done) {
      ...
      product
        ...
        .then(doc => {
          request(app)
            .delete('/products/' + doc._id.toString() + '/delete')
            .set('Authorization', 'Bearer ' + token)
            .send()
            .end(function(err, res) {
    ...
    it('should NOT delete a product', function(done) {
      ...
      request(app)
        .put('/products/INVALID_ID/delete')
        .set('Authorization', 'Bearer ' + token)
        .send(new_values)
        .end(function(err, res) {
          expect(res.statusCode).to.equal(404);
